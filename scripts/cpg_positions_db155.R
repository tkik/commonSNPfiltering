...
anno<-rnb.get.annotation(type = "probes450", assembly = "hg19")
...
saveRDS(combined_all_overlap, file = file.path(out.folder, "cg_o_SNP_155.RDS"))

saveRDS(combined_all_overlap_3, file = file.path(out.folder, "cg_o3_SNP_155.RDS"))

cgs<-data.frame(rownames(combined_all_overlap))
write.table(cgs, file = file.path(out.folder, "blacklist_overlapping_SNP_155.txt"), sep="\t", row.names=F, col.names=F, quote=F)

cgs_3<-data.frame(rownames(combined_all_overlap_3))
write.table(cgs_3, file = file.path(out.folder, "blacklist_overlapping_SNP_3_155.txt"), sep="\t", row.names=F, col.names=F, quote=F)
